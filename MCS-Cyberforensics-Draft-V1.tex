...
Any cyber-attack against SCADA systems demands forensic investigation to understand the cause and effects of the intrusion or disruption on such systems. Therefore, SCADA system forensics is an essential process within the cyber-security lifecycle that not only helps to identify the cause of an incident and those responsible but to help develop and design more secure CPS and critical infrastructure systems of the future. However, a SCADA system has a critical requirement of being continuously operational and therefore a forensic investigator cannot power-off the MCS system for data acquisition and analysis. 

However, MCS being mission critical are required to be operational continuously with zero-tolerance for downtime. Consequently, incident responsers and forensics analysts cannot power off the system for traditional forensics data acquisition. Conseqently, live acquisition is the most logical solutions approach. One other complimentary technique is network and fieldbus traffic analysis and correlation of evidentary information across memory dump and packet captures for a more robust incident response and forensics analysis. 
\begin{table*}[!h]
\centering
...
\item On December 23, 2015, Ukrainian power companies experienced unscheduled power outages with a significant impact. Furthermore, there have also been reports of traces of malware found in a variety of critical infrastructure sectors across Ukrainian. The \textit{BlackEnergy} (BE) malware has been speculated as the culprit which is believed to have been delivered via spear phishing emails with malicious Microsoft Office attachments. Following the attacks, it was also confirmed that the actors wiped the master boot record of some systems by executing the \textit{KillDisk} malware as well as the targeted deletion of some logs at the conclusion of the cyber-attack~\cite{ukrain-2015, symantec-ukrain}.
...
Fieldbus is the name of a family of industrial computer network protocols used for real-time distributed control, standardized as IEC 61158~\cite{wiki-fieldbus}. Fieldbus works on a network structure which typically allows daisy-chain, star, ring, branch, and tree network topologies.
...
\item \textbf{High Degree of Proprietary Solutions and Heterogeneity} Not only are the solutions proprietary in nature due to the unique domain requirement of each and every control systems environment, but also there degree of heterogeneity among field devices even within the same domain is significant to have a ``one-size-fits-all'' solution. Such proprietary solutions with such heterogeneity will make live forensics extremely challenging some of which are as follows -- network constraints due to OEM restrictions, node/device constraints -- due to variations in HW/SW, communications security €“due to legacy and resource constrained devices and proprietary protocols, and the likes.
\end{enumerate}

Among contemporary state-of-the-art forensics tools with live forensics capabilities, two tools viz -- \textit{Volatility} and \textit{Rekall} -- have markedly better capabilities and performance. Despite their populatiry as current best choice solutions, both these tools currently offer very limited support for ARM and PowerPC (PPC) based systems. This is due to the fact that both \textit{Volatility} and \textit{Rekall} have been primarily designed and developed around the \textit{x86} systems. 

In our on going broad research efforts in the areas of live forensics in SCADA-MCS systems, we have researched into the possibility of expanding either of these tools to support live forensics on ARM/PPC based SCADA-MCS systems. While it is certainly achievable, the level of effort for porting is significant, especially taking into considerations. This is particulary amplified when factoring the subtle variations implemented by vendors in a bid to create proprietary solutions seeking edge over competitors. 

\begin{figure}[!t]
\subfigure[MCS with PROFIBUS]{
\includegraphics[scale=0.22]{figures/profibus.eps}}
\subfigure[MCS with MODBUS]{
\includegraphics[scale=0.22]{figures/modbus.eps}}
\caption{SCADA-MCS systems architecture with two popular fieldbus protocols.}
\label{fig:fieldbus}
\end{figure}
...
\item Observing PMem's behavior when we try to append a new memory dump to an existing one. This is the default behavior for when the specified output file already exists.
\end{enumerate}
...
\section{Forensics Tools Empirical Validation}\label{sec:results}
...
Cyberforensics capabilities and readiness in Machinery Control Systems' environment is a fundamental requirement within the cyber-security lifecycle that not only helps to identify the cause of an incident and traceback the perpeterators, but also helps develop and design more secure CPS and critical infrastructure systems of the future as part of a more robust cyberdefense strategy. However, MCS being mission critical are required to be operational continuously with zero-tolerance for downtime. Consequently, incident responsers and forensics analysts cannot power off the system for traditional forensics data acquisition. Conseqently, live acquisition is the most logical solutions approach. One other complimentary technique is network and fieldbus traffic analysis and correlation of evidentary information across memory dump and packet captures for a more robust incident response and forensics analysis. 
