...
\title{Empirical Evaluation of Suitability of Existing Forensics Solutions to Support Live Forensics on SCADA Mchinery Control Systems}
...
\begin{abstract}
SCADA-based Machinery Control Systems (SCADA-MCSs) from leading vendors today are more vulnerable than ever before. To further complicate the secure and reliable operations of these systems, their cyber-physical environments open a whole new world of vulnerabilities which can be readily exploited, many of which are freely available on the Internet. SCADA-MCS being mission critical, must be continuously operational with zero downtime. Consequently, a forensic investigator cannot shut down the system to capture evidentiary data and analyze them. In such scenarios where there is zero downtime tolerance, live forensics is the only viable solution for conducting a digital investigation. Live forensics involves performing data acquisition and analysis on a running system. This will allow for a complete and detailed analysis while the system is still up and running, therefore having little to no effect on the mission.

In this paper, we evaluate some of the most prpolar and powerful forensics tools and frameworks to determine their suitability for porting to SCADA environment, specifically for MCS. We design and develop a rudimentary prototype of a live forensics framework for SCADA-MCS that can readily integrate the existing tools and techniques on to a common platform. Our prototype is designed conforming to \textit{Open Systems Architecture} (OSA) to support cross vendor and cross platform solutions. Our empirical analysis highlights the strengths and weakness of tools studied proposes other requisite capabilities for tools to support a comprahensive live forensics on SCADA-MCS.
\end{abstract}
...
\subsection{ProfiBus -- A Review}
Process Field Bus -- PROFIBUS -- is a standard for fieldbus communication in automation technology and used by Siemens. PROFIBUS is standardized in IEC 61158 and is the world's most successful fieldbus with 53.7 million devices installed by the end of 2015. Utilizing a single, standardized, application-independent communication protocol, PROFIBUS supports fieldbus solutions both in factory and process automation as well as in motion control and safety-related tasks.

%PROFINET is an open Industrial Ethernet standard developed by the PROFIBUS Organisation. PROFINET is completely standard Ethernet (IEEE802.3) and operates at 100Mbit/s over twisted-pair copper or fibre- optic cables making use of TCP/IP and other IT standards for non-real- time communications such as configuration and parameters. They also provides a ``real-time'' channel for time-critical communications such as process data.
...
\begin{itemize}
\item Proprietary tools are rigid due to customized solutions, have limited capabilities, and are prohibitively expensive. 

\item APIs available on the target SCADA system for integrating live forensics tools are limited.

\item Support for expand existing open source forensics tools to support SCADA live forensics are minimum.
